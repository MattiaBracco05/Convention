...
<?php
    ...
    if (isset($_GET['ID'])) {
        ...
        $queryRicavaIDPart =
            "SELECT IDPart
            FROM Partecipante
            WHERE IDUtente = '$varUtente'";
        
        //Eseguo la query (elimino le righe dalla tabella "Sceglie" dove "IDPart" è associato al partecipante dell'utente da eliminare)
        $risultato = Database :: eseguiQuery($queryRicavaIDPart);

        //Quando ottengo il risultato -->
        if ($risultato) {
            //Ricavo l'ID del Partecipante
        ...
        //Eseguo la query (elimino le rrighe nella tabella sceglie)
        Database :: eseguiQuery($queryEliminaSceglie);

        //Creo la query da eseguire
        $queryEliminaPartecipantepante =
            "DELETE FROM Partecipante
            WHERE IDUtente = $varUtente";
        
        //Eseguo la query (elimino il partecipante)
        Database :: eseguiQuery($queryEliminaPartecipantepante);
        ...
        //Eseguo la query (elimino l'utente) --> (sweet alert)
        if (Database :: eseguiQuery($queryEliminaUtente)) {
            echo '<script>';
            echo 'document.addEventListener("DOMContentLoaded", function() {';
            echo '    Swal.fire({';
            echo '        icon: "info",';
            echo '        title: "Utente bannato!",';
            echo '        text: "Hai appena eliminato l\'utente dal database.",';
        ...
        //Altrimenti --> Messaggio di errore (nell'eliminazione dell'utente) (sweet alert)
        else {
            echo '<script>';
            echo '    document.addEventListener("DOMContentLoaded", function() {';
            echo '        Swal.fire({';
