...
<?php
    ...
    if (isset($_GET['IDRel'])) {
        ...
        $queryVerifica =
            "SELECT COUNT(*) AS numProgrammi
            FROM Relatore
            JOIN Relaziona ON Relatore.IDRel = Relaziona.IDRel
            WHERE Relatore.IDRel = '$varRelatore'";
        
        //Eseguo la query (verifico se ci sono programmi associati al relatore che si vuole eliminare)
        $risultato = Database :: eseguiQuery($queryVerifica);
        
        //Controllo i risultati (ricavo il numero di programmi associati)
        if ($risultato) {
            $row = $risultato -> fetch_assoc();
            $numProgrammi = $row["numProgrammi"];
        
            //Se ci sono programmi associati --> Messaggio di errore (sweet alert)
            if ($numProgrammi > 0) {
                echo '<script>';
                ...
                echo '            text: "Impossibile eliminare il relatore in quanto è ancora associato a dei programmi.",';
            ...
            //Altrimenti --> Non ci sono relatori associati --> Procedo con l'eliminazione
            else {    
                //Creo la query da eseguire
                $queryCambiaTipologia =
                    "UPDATE Partecipante
                    SET TipologiaPart = 'Base'
                    WHERE IDPart IN (
                        SELECT p.IDPart
                        FROM Partecipante p
                        JOIN Relatore r ON p.MailPart = r.MailRel
                        WHERE r.IDRel = '$varRelatore'
                    );";

                //Eseguo la query (cambio la tipologia del partecipante in "Base")
                Database :: eseguiQuery($queryCambiaTipologia);
                
                //Creo la query da eseguire
                $queryEliminaRelatore =
                    "DELETE FROM Relatore
                    WHERE IDRel = '$varRelatore'";
        
                //Eseguo la query (elimino il relatore) --> (sweet alert)
                if (Database :: eseguiQuery($queryEliminaRelatore)) {
                    echo '<script>';
                    echo '    document.addEventListener("DOMContentLoaded", function() {';
                    echo '        Swal.fire({';
                    echo '            icon: "info",';
                    echo '            title: "Relatore Eliminato!",';
                    echo '            text: "Il relatore è stato cancellato dal database.",';
                    echo '            confirmButtonColor: "#3085d6",';
                    echo '            confirmButtonText: "OK"';
                    echo '        }).then(function() {';
                    echo '            window.location.href = "./homeAdmin.php";';
