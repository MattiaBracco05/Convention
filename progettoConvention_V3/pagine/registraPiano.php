...
<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        //Eseguo la query (verifico che il piano non sia già presente)
        $risultato = Database :: eseguiQuery($queryVerifica);

        //Controllo il risultato --> Se il piano è già presente nel database --> Messaggio di errore (sweet alert)
        if ($risultato && $risultato->num_rows > 0) {
            echo '<script>';
            echo 'document.addEventListener("DOMContentLoaded", function() {';
            echo '    Swal.fire({';
            echo '        icon: "error",';
            echo '        title: "Errore di costruzione!",';
            echo '        text: "È già presente questo piano.",';
        ...
        //Altrimenti --> Inserisco l'azienda nel database
            ...
            //Creo la query da eseguire
            $queryInserisciPiano =
                "INSERT INTO Piano (Numero)
                VALUES ('$numero')";

            //Eseguo la query (inserisco l'azienda) --> (sweet alert)
            if (Database :: eseguiQuery($queryInserisciPiano)) {
                echo '<script>';
                echo 'document.addEventListener("DOMContentLoaded", function() {';
                echo '    Swal.fire({';
                echo '        icon: "success",';
                echo '        title: "Costruione effettuata!",';
                echo '        text: "Hai completato la costruzione del piano.",';
                echo '        confirmButtonColor: "#3085d6",';
                echo '        confirmButtonText: "OK"';
                echo '    }).then(function() {';
                echo '        window.location.href = "./homeAdmin.php";';
