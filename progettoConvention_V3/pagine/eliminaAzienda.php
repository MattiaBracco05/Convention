...
<?php
    ...
    if (isset($_GET['RagioneSociale'])) {
        ...
        $varAzienda = $_GET['RagioneSociale'];
        ...
        $queryVerifica =
            "SELECT COUNT(*) AS numRelatori
            FROM Relatore
            WHERE RagioneSocialeFK = '$varAzienda'";
        
        //Eseguo la query (verifico se ci sono relatori associati all'azienda che si vuole eliminare)
        $risultato = Database :: eseguiQuery($queryVerifica);
        
        //Controllo i risultati (ricavo il numero di relatori associati)
        if ($risultato) {
            $row = $risultato->fetch_assoc();
            $numRelatori = $row["numRelatori"];
        
            //Se ci sono relatori associati --> Messaggio di errore (sweet alert)
            if ($numRelatori > 0) {
                echo '<script>';
                ...
                echo '            text: "Impossibile eliminare l\'azienda in quanto sono ancora presenti relatori associati ad essa.",';
            ...
            //Altrimenti --> Non ci sono relatori associati --> Procedo con l'eliminazione
                ...
                $queryEliminaAzienda =
                    "DELETE FROM Azienda
                    WHERE RagioneSociale = '$varAzienda'";
        
                //Eseguo la query (elimino l'azienda) --> (sweet alert)
                if (Database :: eseguiQuery($queryEliminaAzienda)) {
                    echo '<script>';
                    echo '    document.addEventListener("DOMContentLoaded", function() {';
                    echo '        Swal.fire({';
                    echo '            icon: "info",';
                    echo '            title: "Azienda Eliminata!",';
                    echo '            text: "L\'azienda è stata cancellata dal database.",';
                    echo '            confirmButtonColor: "#3085d6",';
                    echo '            confirmButtonText: "OK"';
                    echo '        }).then(function() {';
                    echo '            window.location.href = "./homeAdmin.php";';
