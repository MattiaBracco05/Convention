...
<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $ragioneSociale = ucwords(strtolower($ragioneSociale));
        ...
        $queryVerifica =
            ...
            FROM Azienda
            WHERE RagioneSociale = '$ragioneSociale'";
        
        //Eseguo la query (verifico che l'azienda non sia già presente)
        $risultato = Database :: eseguiQuery($queryVerifica);

        //Controllo il risultato --> Se l'azienda è già presente nel database --> Messaggio di errore (sweet alert)
        if ($risultato && $risultato->num_rows > 0) {
            echo '<script>';
            echo 'document.addEventListener("DOMContentLoaded", function() {';
            echo '    Swal.fire({';
            echo '        icon: "error",';
            echo '        title: "Errore di registrazione!",';
            echo '        text: "È già stata registrata un\'azienda con questa ragione sociale.",';
        ...
        //Altrimenti --> Inserisco l'azienda nel database
        else {
            //Creo la query da eseguire
            $queryInserisciAzienda =
                "INSERT INTO Azienda (RagioneSociale, IndirizzoAzienda, TelefonoAzienda)
                VALUES ('$ragioneSociale', '$indirizzo', '$telefono')";

            //Eseguo la query (inserisco l'azienda) --> (sweet alert)
            if (Database :: eseguiQuery($queryInserisciAzienda)) {
                echo '<script>';
                echo 'document.addEventListener("DOMContentLoaded", function() {';
                echo '    Swal.fire({';
                echo '        icon: "success",';
                echo '        title: "Registrazione effettuata!",';
                echo '        text: "Hai completato la registrazione dell\'azienda.",';
                echo '        confirmButtonColor: "#3085d6",';
                echo '        confirmButtonText: "OK"';
                echo '    }).then(function() {';
                echo '        window.location.href = "./homeAdmin.php";';
            ...
            //Altrimenti --> Messaggio di errore (inserimento dell'azienda) (sweet alert)
            else {
                echo '<script>';
                echo '    document.addEventListener("DOMContentLoaded", function() {';
                echo '        Swal.fire({';
