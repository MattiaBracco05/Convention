...
<?php
    ...
    if (isset($_GET['IDSpeech'])) {
        
        //Ricavo il valore del parametro
        $varSpeech = $_GET['IDSpeech'];
        ...
        $queryVerifica =
            ...
            FROM Programma
            WHERE IDSpeech = '$varSpeech'";

        //Eseguo la query (verifico se ci sono programmi associati allo speech che voglio eliminare)
        ...
        if ($risultato) {
            ...
            //Altrimenti --> Non ci sono programmi associati --> Procedo con l'eliminazione
            else {    
                //Creo la query da eseguire
                $queryEliminaSpeech =
                    "DELETE FROM Speech
                ...
                //Eseguo la query (elimino lo speech) --> (sweet alert)
                if (Database :: eseguiQuery($queryEliminaSpeech)) {
                    echo '<script>';
                    echo 'document.addEventListener("DOMContentLoaded", function() {';
                    echo '    Swal.fire({';
                    echo '        icon: "info",';
                    echo '        title: "Speech Eliminato!",';
                    echo '        text: "Hai eliminato lo speech dal database.",';
                    echo '        confirmButtonColor: "#3085d6",';
                    echo '        confirmButtonText: "OK"';
                    echo '    }).then(function() {';
                    echo '        window.location.href = "./homeAdmin.php";';
                ...
                //Altrimenti --> Messaggio di errore (nell'eliminazione dello speech) (sweet alert)
                else {
                    echo '<script>';
                    echo '    document.addEventListener("DOMContentLoaded", function() {';
                    echo '        Swal.fire({';
