...
<?php
    ...
    //Controllo se i parametri "IDPart" e "IDProgramma" sono stati passati tramite l'URL, se sì -->
    if (isset($_GET['IDPart']) && isset($_GET['IDProgramma'])) {
        ...
        $queryAnnullaIscrizione =
            "DELETE FROM Sceglie
            WHERE IDProgramma = '$varProgramma' AND IDPart = '$varPart';";

        //Eseguo la query (annullo l'iscrizione) --> (sweet alert)
        if (Database :: eseguiQuery($queryAnnullaIscrizione)) {
            echo '<script>';
            echo 'document.addEventListener("DOMContentLoaded", function() {';
            echo '    Swal.fire({';
            echo '        icon: "info",';
            echo '        title: "Iscrizione annullata!",';
            echo '        text: "Per partecipare a questo evento dovrai iscriverti nuovamente.",';
        ...
        //Altrimenti --> Messaggio di errore (nell'annullare l'iscrizione) (sweet alert)
        else {
            echo '<script>';
            echo '    document.addEventListener("DOMContentLoaded", function() {';
            echo '        Swal.fire({';
            echo '            icon: "error",';
            echo '            title: "Errore nell\'annullamento!",';
            echo '            text: "Si è verificato un problema nell\'annullare l\'iscrizione.",';
            echo '            confirmButtonColor: "#3085d6",';
            echo '            confirmButtonText: "OK"';
            echo '        }).then(function() {';
            echo '            window.location.href = "./homeUtente.php";';
