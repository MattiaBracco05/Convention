...
<?php
    ...
    if (isset($_GET['IDProgramma'])) {
        ...
        $queryEliminaProgramma =
            "DELETE FROM Relaziona
            WHERE IDProgramma = '$varProgramma';";

        //Eseguo la query (elimino il programma anche dalla tabella relaziona)
        if (Database :: eseguiQuery($queryEliminaProgramma)) {
        }

        //Altrimenti --> Messaggio di errore (nell'eliminazione del programma)
        else {
            echo "\nErrore nell'eliminazione del programma! (tab. Relaziona)";
        }
        ...
        //Eseguo la query (elimino il programma anche dalla tabella sceglie)
        if (Database :: eseguiQuery($queryEliminaProgramma)) {
            //Query eseguita con successo
        }

        //Altrimenti --> Messaggio di errore (nell'eliminazione del programma) (sweet alert)
        else {
            echo '<script>';
        ...
        //Eseguo la query (elimino il programma) --> (sweet alert)
        if (Database :: eseguiQuery($queryEliminaProgramma)) {
            echo '<script>';
            echo 'document.addEventListener("DOMContentLoaded", function() {';
            echo '    Swal.fire({';
            echo '        icon: "info",';
            echo '        title: "Programma Eliminato!",';
            echo '        text: "Hai eliminato il programma dal database.",';
            echo '        confirmButtonColor: "#3085d6",';
            echo '        confirmButtonText: "OK"';
            echo '    }).then(function() {';
            echo '        window.location.href = "./homeAdmin.php";';
